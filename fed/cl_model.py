...
from spirl.utils.pytorch_utils import LossSpikeHook, NanGradHook, NoneGradHook, update_optimizer_lr, RAdam, SAM , ASAM , ASOL
...
class BaseClients(BaseTrainer,fl.client.NumPyClient):
    def __init__(self, args):
        ...
        #self.model.create_beta_cycle(cycle_length = 5, min_beta=1e-3, max_beta=1e-1)
    ...
    def _default_hparams(self):
        default_dict = ParamDict({
            ...
            'momentum': 0.9,      # momentum in RMSProp / SGD optimizer
            'rho' : 0.05, # SAM/ ASAM optimizer
            'eta' : 0.02, # ASAM optimizer
            'mu' : 0.0005, #Proximal term
...
class ASAMClients(BaseClients):
    def __init__(self, args):
        super().__init__(args)
        self.minimizer = eval(self._hp.minimizer)(self.optimizer, self.model, rho=self._hp.rho, eta=self._hp.eta)
    ...
    def fit(self, parameters, config):
        logging.debug("Fitting the model with new parameters")
        self.set_parameters(parameters)
        if "learning_rate" in config:
            update_optimizer_lr(self.optimizer ,config["learning_rate"])
        self.train()
        return self.get_parameters(config), len(self.train_loader), {}
...
class FP_Clients(BaseClients):
    ...
    def add_loss_drift(self,losses):
        ...
        exception_list = ["num_batches_tracked", "p.0", "running_mean", "running_var"]
        prox_term = 0.0
...
class FS_Clients(BaseClients):
    ...
    def __init__(self, args):
        ...
        if self._hp.minimizer == "ASOL":
            self.minimizer = eval(self._hp.minimizer)(optimizer =self.optimizer,model = self.model, global_param_dict = self.global_weights ,rho=self._hp.rho)
        elif self._hp.minimizer == "SAM":
            self.minimizer = eval(self._hp.minimizer)(optimizer =self.optimizer,model = self.model, rho=self._hp.rho)
