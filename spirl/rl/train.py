...
class RLTrainer:
    ...
    def __init__(self, args):
        ...
        # build env /// env count should be upper, add config to more env to add
        ...
        try:
            if self.conf.env.Mul_tasks : self.Multi_env_attr()
        except:
            self.env = [self._hp.environment(self.conf.env)]
        self.conf.agent.env_params = self.env[0].agent_params      # (optional) set params from env for agent
        if self.is_chef:
    ...
    def train_epoch(self, epoch):
        ...
        #///make sampler more and more steps recovery
        for sampler in self.sampler:
            sampler.init(is_train=True)
            ...
            while self.global_step - ep_start_step < self._hp.n_steps_per_epoch:
                with timers['batch'].time():
                    ...
                    with timers['rollout'].time():
                        experience_batch, env_steps = sampler.sample_batch(batch_size=self._hp.n_steps_per_update,
                                                                                global_step=self.global_step)
    ...
    def val(self):
        """Evaluate agent."""
        # update many env
        for sampler,task_name in zip(self.sampler,self.conf.env.tasks_name):
                ...
                with torch.no_grad():
                    with timing("Eval rollout time: "):
                        for _ in range(WandBLogger.N_LOGGED_SAMPLES):   # for efficiency instead of self.args.n_val_samples
                            val_rollout_storage.append(sampler.sample_episode(is_train=False, render=False))

            rollout_stats = val_rollout_storage.rollout_stats()
            if self.is_chef:
                with timing("Eval log time: "):
                    log_rollout_stats = prefix_dict(rollout_stats, task_name + '_')
                    self.agent.log_outputs(log_rollout_stats, val_rollout_storage,
                                        self.logger, log_images=False, step=self.global_step)
                print("{}_Evaluation Avg_Reward: {}".format(task_name, rollout_stats.avg_reward))
    ...
    def warmup(self):
        ...
        with self.agent.rand_act_mode():
            self.sampler[0].init(is_train=True)
            warmup_experience_batch, _ = self.sampler[0].sample_batch(
    ...
    def setup_logging(self, conf, log_dir):
        if not self.args.dont_save:
            ...
            if self.args.mode == 'train':
                ...
                elif self._hp.logging_target == 'wandb':
                        exp_name = f"{os.path.basename(self.args.path)}_{self.args.prefix}" if self.args.prefix \
                        else os.path.basename(self.args.path)
                        logger= WandBLogger(exp_name, WANDB_PROJECT_NAME, entity=WANDB_ENTITY_NAME,
                                            path=self._hp.exp_path, conf=conf)
    ...
    def print_train_update(self, epoch, agent_outputs, timers):
        ...
        print('Train Epoch: {} [It {}/{} ({:.0f}%)]'.format(
            epoch, self.global_step, self._hp.n_steps_per_epoch * self._hp.num_epochs,
                                     100. * self.global_step / ( 10 *self._hp.n_steps_per_epoch * self._hp.num_epochs)))
        ...
        togo_train_time = timers['batch'].avg * (10 *self._hp.num_epochs * self._hp.n_steps_per_epoch - self.global_step) \
                          / self._hp.n_steps_per_update / 3600.
        print('ETA: {:.2f}h'.format(togo_train_time))

    #making multi_env to train
    def Multi_env_attr(self):
        self.env = []
        for task_index, tasks in enumerate(self.conf.env.tasks_name):
            print(tasks)
            dummy_env_config = AttrDict(
                n_total_tasks=self.conf.env.n_total_tasks,
                task_index=task_index,
                name = tasks,
                seed = self.conf.env.seed
            )
            self.env.append(self._hp.environment(dummy_env_config))
