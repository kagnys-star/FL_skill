...
class ModelTrainer(BaseTrainer):
    ...
    def train(self, parameters) :
        for epoch in range(self.start_epoch, self._hp.num_epochs):
            ...
            #},  os.path.join(self._hp.exp_path, 'weights'), CheckpointHandler.get_ckpt_name(self.epoch))
    ...
    def train_epoch(self, epoch, parameters):
        ...
        for self.batch_idx, sample_batched in enumerate(self.train_loader):
            ...
            if log_len == self.batch_idx :
                self.model.log_outputs(output, inputs, losses, self.grap_round,
            ...
            with autograd.no_grad():
                ...
                if not self.args.dont_save:
                    if self.evaluator is not None:
                        self.evaluator.dump_results(self.grap_round)

                    self.model_test.log_outputs(output, inputs, losses_meter.avg, self.grap_round,
    ...
    def get_dataset(self, args, model, data_conf, phase, n_repeat, dataset_size=-1):
        ...
        loader = dataset_class(self.data_dir, data_conf, resolution=model.resolution,
                               phase=phase, shuffle=phase == "train", dataset_size=dataset_size). \
            get_data_loader(self._hp.batch_size, n_repeat)
        return loader
    
    """ remodeling need
    def parameters_load(self, params):
        self.model.load_state_dict(params, strict=self.args.strict_weight_loading)
