...
class INFOSPiRLMdl(ClSPiRLMdl):
    ...
    def loss(self, model_output, inputs):
        ...
        losses.lablel_loss = CEELoss(self._hp.label_weights)(model_output.labels, torch.argmax(inputs.labels, dim=-1))
...
class ClassSPiRLMdl(ClSPiRLMdl):
    """SPiRL model with closed-loop low-level skill decoder."""
    def build_network(self):
        super().build_network()
        self.c = LinearClassifier(2 * self._hp.nz_vae , self._hp.tasks)
    ...
    def forward(self, inputs, use_learned_prior=False):
        output = super().forward(inputs, use_learned_prior)
        condition_inputs = torch.cat((output.z, output.z_p), dim=-1)
        output.labels = self.c(condition_inputs)
