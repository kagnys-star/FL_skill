...
class Prox_clients(ClSPiRLMdl):

    def loss(self, model_output, inputs, global_params , mu):
        losses = AttrDict()
        ...
        exception_list = ["num_batches_tracked", "p.0"]

        # Initialize as lists to collect tensor elements
        local_param_list = []
        global_param_list = []
        ...
        for key in local_weights.keys():
            ...
            local_param_list.append(local_weights[key].view(-1))
            global_param_list.append(global_params[key].view(-1))

        # Convert lists to tensors using torch.cat for concatenation
        local_param = torch.cat(local_param_list)
        global_param = torch.cat(global_param_list)

        losses.proximal = L2Loss(mu)(local_param, global_param)
